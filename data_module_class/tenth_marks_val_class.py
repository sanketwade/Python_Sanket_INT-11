...
class TenthMarksValidator:
    def __init__(self, user):
        self.user = user
        log_folder = "D:\\Bizmetric Intership\\logs"
        os.makedirs(log_folder, exist_ok=True)
        log_file_name = os.path.join(log_folder, f"login Id_\'\'{self.user}\'\' tenth_marks_validation.log")
        self.logger = configure_logging(log_file_name)

    def validate_tenth_percentage(self, percentage):
        try:
...
if __name__ == "__main__":
    ...
    validator = TenthMarksValidator(user)
    percentage_input = float(input("Enter your percentage: ").strip())
    if validator.validate_tenth_percentage(percentage_input):
